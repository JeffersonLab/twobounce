...
class polygon:
    ...
    def antiClockwise(self, pts):
        ...
        min_p = min(pts, key=getAngle)  # get the value with the min angle
        min_p_i = pts.index(min_p)  # get the index of such point
        rotate_arr = (  # Rotate the array so that the min point is in front
            pts[min_p_i:] + pts[:min_p_i]
        )

        # Make sure the rotated array is always increasing. If not, the point are not defined in an anti-clockwise manner.
        prev = getAngle(min_p)  # Set initial value
        for pt in rotate_arr[1:]:
            if getAngle(pt) < prev:
                if ANTICLOCKWISE_WARNING:
                    print(
                        "Warning: Geometry not anti-clockwise. Automatically fixing..."
                    )
                else:
                    raise polyError("Points not defined in an anti-clockwise manner")
                self.pts = sorted(pts, key=getAngle)
                return False
            prev = getAngle(pt)
        return True
